<x-app-layout>
    ...
    <div class="bg-gray-100 p-4">
        <div class="bg-white pb-4 rounded-xl">
            ...
            {{-- Icons Info --}}
            <div class="flex items-center justify-around mt-3 text-lg">
                <div class="flex items-center gap-2">
                    ...
                    <p class="m-0">{{ \Carbon\Carbon::parse($event->start)->format('F j') }}</p>
                </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <p class="m-0">{{ \Carbon\Carbon::parse($event->start)->format('g:i A') }}</p>
                </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <p class="m-0">{{count($event->bookings)}}</p>
                </div>
            ...
            @if (count($event->bookings))
                {{-- Table Info --}}
                <div class="mt-4 bg- p-4 rounded">
                    <h2 class="underline mb-4 ml-4">People Attending : </h2>
                    <table class="w-full ">
                        <thead>
                            <tr class="font-semibold text-xl border-b-2 border-black">
                                <td>Name</td>
                                <td>Email</td>
                                <td>Gender</td>
                                <td>Booked At</td>
                        ...
                        <tbody>
                            @foreach ($event->bookings as $key => $visitor)
                                <tr class="h-[7vh] capitalize">
                                    <td>{{ $visitor->full_name }}</td>
                                    <td>{{ $visitor->email }}</td>
                                    <td>
                                        <span

                                        class="text-sm rounded-full px-2 py-1 capitalize {{$visitor->gender == 'male' ? 'bg-sky-100' : 'bg-pink-100'}}">{{$visitor->gender}}</span>
                                    </td>
